...
public class TerminalOperationMain {
    ...
    public static void main(String[] args) {
        ...
        // Collectors는 뒤에서 더 자세히 (뽁잡한 수집이 필요할 때 사용)
        System.out.println("1. collect - List 수집");
        List<Integer> evenNubers1 = numbers.stream()
                .filter(n -> n % 2 == 0)
                .collect(Collectors.toList());
        System.out.println("짝수 리스트: " + evenNubers1);
        System.out.println();

        System.out.println("2. toList() - (Java 16+)");
        List<Integer> evenNubers2 = numbers.stream()
                .filter(n -> n % 2 == 0)
                .toList();
        System.out.println("짝수 리스트: " + evenNubers2);
        System.out.println();

        System.out.println("3. toArray - 배열로 변화");
        ...
        boolean noNegavtive = numbers.stream()
                .noneMatch(n -> n < 0);
        System.out.println("음수가 없나? " + noNegavtive);
    }
}
